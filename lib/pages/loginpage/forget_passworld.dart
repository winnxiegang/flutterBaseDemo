...
class ForgetPassWorldPageState extends State<ForgetPassWorldPage> {
  ...
  void reGetCountdown() {
    if (usePhoneController.text.trim().length != 11) {
      return ToastOk.show(msg: "请输入正确的手机号");
    }
    _getPassCode();
    if (mounted) {
      setState(() {
  ...
  Future _getPassWorld() async {
    DioRequestControl.getInstans()
        .getPassWorld(usePhoneController.text.trim(),
            useCodeController.text.trim(), context)
        .then((value) {
  ...
  Future _getPassCode() async {
    DioRequestControl.getInstans()
        .getPassCode(usePhoneController.text.trim(), context)
        .then((value) {});
  }
}
